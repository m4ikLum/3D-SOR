/****************************************************************************
   3D SOR Implementation (Parallel Version)
...
int main(int argc, char *argv[])
{
  ...
  printf("3D SOR Parallel Implementation\n");
...
//3D SOR - Parallel implementation
void SOR3D_parallel(cube_ptr c, int *iterations, int num_threads)
{
  pthread_t *threads;
  thread_args *args;
...
//thread worker function for SOR computation
void *SOR3D_thread_work(void *arg)
{
  ...
  //calculate work division for k-slices (z-coordinate)
  int k_points = n - 2;  //exclude ghost zone
  int k_per_thread = k_points / num_threads;
  int k_remainder = k_points % num_threads;
  
  //calculate range of k values 
  int k_start = 1 + thread_id * k_per_thread;
  ...
  while (*continue_iteration) {
    ...
    *my_change = 0.0;
    ...
    for (k = k_start; k < k_end; k++) {        //z-coordinate 
