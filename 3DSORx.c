/****************************************************************************
   3D SOR Implementation (Parallel Version - X-axis Partitioning)
...
typedef struct {
  ...
  data_t *data;    //3D data stored as 1deeznnutz (jk) array
} cube_rec, *cube_ptr;
...
int main(int argc, char *argv[])
{
  ...
  printf("3D SOR Parallel Implementation (X-axis Partitioning)\n");
...
//3D SOR - Parallel implementation with X-axis partitioning 
void SOR3D_parallel(cube_ptr c, int *iterations, int num_threads)
{
  pthread_t *threads;
  thread_args *args;
  ...
  //WAIT (they dont love me like i love you) for all threads to complete 
  for (t = 0; t < num_threads; t++) {
...
//thread worker function for SOR computation - x-axis partitioning
void *SOR3D_thread_work(void *arg)
{
  ...
  //calculate work division for i-slices (x-coordinate)
  int i_points = n - 2;  //exclude ghost zone
  int i_per_thread = i_points / num_threads;
  int i_remainder = i_points % num_threads;
  
  //calculate range of i values 
  int i_start = 1 + thread_id * i_per_thread;
  int i_end = i_start + i_per_thread;
    ...
    *my_change = 0.0;
    ...
    for (k = 1; k < n-1; k++) {          //z-coordinate 
      for (j = 1; j < n-1; j++) {        //y-coordinate 
        for (i = i_start; i < i_end; i++) {  //x-coordinate (partitioned)
          idx = idx3D(i, j, k, n);
